...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/youtube") {
      ...
      // "Extended Highlights" cut wins when one exists. Used by the 2nd highlight
      // button (soccer) to serve a reliably-distinct video from the standard cut
      // the 1st button already plays — "normal + extended" (Jacob 7/7). Falls back
      // to the standard ordering when no extended video matched, so it's never worse.
      ...
      try {
        ...
        if (preferChannel) {
          ...
          // When prefer=extended, the SAME tiers run but the extended cut of
          // each tier is checked before its standard sibling — so the longer
          // "Extended Highlights" wins if it exists, else we fall straight back
          // to the standard ordering (never returns fewer results than default).
