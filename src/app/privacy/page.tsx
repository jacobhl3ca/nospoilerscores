  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-10 text-[15px] leading-relaxed" style={{ color: "var(--text)" }}>
      ...
      <section className="space-y-4">
        ...
        <p>
          If you are signed in, we also tag your analytics session with a random, scrambled account code and whether you are using the website or the iPhone app, so we can tell how many real accounts &mdash; rather than how many devices &mdash; use each one. That code is derived from your account with a one-way key we keep private: it cannot be turned back into your email or your Apple or Google ID, and it is only ever sent to our own self-hosted Umami, never to a third party.
        </p>
        <p>
        ...
        <p>
          HideScore works fully without an account. You can optionally sign in with Apple or Google so your favorite teams and settings sync across your devices. If you choose to sign in, we store the email address associated with that sign-in and your saved preferences on our server, plus a small usage record: when the account was first and last seen and whether it has been used on the website, the iPhone app, or both. That is the only personal data we hold, you can see it yourself under Settings &rarr; Account, and you can sign out at any time. Deleting your account erases all of it.
        </p>
        <p>
