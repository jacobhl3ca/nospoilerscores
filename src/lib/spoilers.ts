...
//     "put(?:s|ting)?[- ]paid[- ]to" catches the British ending idiom recap titles reach for
//     when a result kills off the other side's run or hopes ("Liverpool put paid to Arsenal's
//     unbeaten run", "City puts paid to United's title hopes", "a red card putting paid to the
//     comeback"). It names the beaten side and slipped past the "end …reign / …unbeaten run"
//     family, yet in a highlight title "put paid to" means nothing but a decisive defeat/elimination.
//     "(?:s|ting)?" covers put/puts/putting (past "put" shares the base form) and the two "[- ]"
//     separators cover the spaced or rare-hyphenated form; the phrase has no benign sports-title
//     sense, so the false-positive risk is negligible. Byte-identical to the worker's copy.
...
//     to a role"), so instead it is caught in three anchored forms only. First, a result
...
//     is caught while "promoted to captain"/"promotional video" stay visible. Third, the
//     noun-first result form "promotion secured/confirmed/assured/guaranteed/achieved/
//     sealed/clinched/won/complete(d)" — the exact flip of the first branch, which only
//     matched the verb BEFORE the noun, so a headline that leads with the outcome
//     ("Promotion secured for Leeds", "Promotion confirmed on the final day") had leaked.
//     Anchoring the verb immediately after "promotion" keeps the drop-race PREVIEWS
//     visible ("promotion race"/"promotion hopes"/"promotion picture" match no verb), and
//     "secured/confirmed/assured/guaranteed/achieved" are not standalone win-verbs above
//     (sealed/clinched/won already were, so those forms are belt-and-braces). The
//     "promotion" of a boxing/MMA org ("Matchroom promotion") matches none of the three.
...
//     "decisive[- ](?:goals?|penalt(?:y|ies)|strikes?|touchdowns?|tr(?:y|ies)|baskets?|buckets?|scores?)" sits
//     next to the "go[- ]?ahead …" scoring-play family it mirrors: "the decisive goal", "scores the decisive
//     penalty", "a decisive strike", "the decisive touchdown" all name the match-DECIDING score, so the title
//     reveals a result just as plainly as the already-masked "winner"/"matchwinner"/"game-winning" family does —
//     yet "decisive" was the one such synonym no branch caught, and the phrases carry no digits for SCORE_RX.
//     The MANDATORY trailing scoring noun is what keeps it safe: bare "decisive" is a common non-result word
//     ("decisive action", "a decisive week in the title race", "decisive leadership", "the decisive factor"),
//     but none of those benign uses puts a concrete scoring noun immediately after "decisive", so they stay
//     visible. The set is deliberately confined to the unambiguous scoring plays — the vaguer "decisive run"
//     (a run OF FORM, a run IN the ninth — indistinguishable), "decisive blow"/"decisive breakthrough" (both
//     carry non-scoring senses) are LEFT OUT so a streak-of-form or a negotiation headline can't trip it.
//     "decisive victory"/"decisive win" already match via the bare "victory"/"win" tokens, so they need no
//     branch here. Byte-identical to the worker's copy.
...
//     "successful(?:ly)?[- ]…defen[cs]e"/"successfully[- ]defend(?:s|ed|ing)?" is the sibling of
//     "retain": a title fight whose result is told as a defence rather than a retention leaked,
//     because "retain" caught "retains the belt" but not "Canelo successfully defends his title" /
//     "a successful title defence for Fury" — the fighter who successfully defends WON, so it is the
//     same result reveal. The "successful(?:ly)?" prefix is what makes it safe: a bare "defends"/
//     "title defence" is ambiguous with the preview ("Canelo to defend his title", "how to watch the
//     title defence"), but a defence is only ever called SUCCESSFUL after it has happened, so the
//     adverb/adjective anchors it to a completed outcome and leaves every upcoming-fight preview
//     visible. "defen[cs]e" spans British/American spelling and cannot match "defensive"/"defender".
...
//     breaks the whole promise). Byte-identical to the worker's copy.
//     "(?:take|takes|taking|took) care of business" catches the US-sports "won the game they were
//     supposed to win" idiom NBA/NFL/MLB recaps lean on ("Celtics TAKE CARE OF BUSINESS in Game 2",
//     "Chiefs took care of business against the Broncos", "Dodgers taking care of business to close the
//     series") — a win reveal that names no score for SCORE_RX and matched no existing keyword. It is
//     anchored to the whole "care of business" phrase after a take verb, so "take care of the ball"
//     (protecting possession) and a bare "take care" pass through untouched; only the joined idiom
//     matches. Its one benign over-match — a front-office "take care of business off the field" line —
