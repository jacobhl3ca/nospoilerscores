...
//     "(?:makes?|made|making) (?:light|hard) work of" is the comfortable-/scrappy-win idiom the
//     same family missed: a side that "makes light work of" an opponent won it easily, and one
//     that "made hard work of" it still won (the phrase only ever precedes the beaten side) —
//     the plainest way a soccer/WC recap frames the margin without a scoreline ("Barcelona make
//     light work of Getafe", "City made light work of it", "USA make hard work of Panama") — a
...
//     "claim(?:s|ed|ing)? (?:the )?(?:title|crown|trophy|championship|pennant|silverware)" catches the
...
//     unambiguous silverware object. The contention preview "lay claim to the title" reads "claim to the
//     title" — a "to" splits "claim" from the object — so it never fires; only the achieved result does. Any
//     residual over-hide (a "can they claim the title?" preview) errs to the over-hide-is-safe side (a masked
...
//     Byte-identical to the worker's copy.
const SPOILER_RX = /\b(walk[- ]?off|walk(?:s|ed|ing)?[- ]?it[- ]?off|buzzer[- ]?beaters?|comeback|come[- ]from[- ]behind|(?:come|came)[- ]from(?:[- ](?:an?|\d{1,2}|one|two|three|four|five|six))?(?:[- ](?:goals?|sets?|points?|runs?|scores?))?[- ]down\b|(?:storm|roar|claw)(?:s|ed|ing)?[- ]?back|battl(?:e|es|ed|ing)[- ]?back|peg(?:s|ged|ging)?[- ]?back|(?:pulls?|pulled|pulling|grabs?|grabbed|grabbing) (?:one|a goal|another) back|fight(?:s|ing)?[- ]?back|fought[- ]?back|rall(?:y|ies|ied|ying) (?:past|back|from)|(?:overturn(?:s|ed|ing)?|overhaul(?:s|ed|ing)?|wip(?:e|es|ed|ing)[- ]?out|eras(?:e|es|ed|ing))(?: [\w'’-]+){0,4}? deficit|(?:cut(?:s|ting)?|halv(?:e|es|ed|ing)|reduc(?:e|es|ed|ing)|trim(?:s|med|ming)?|slash(?:es|ed|ing)?)(?: [\w'’-]+){0,3}? (?<!(?:budget|trade|fiscal|spending|wage|wages|federal|national|structural) )deficit|extra[- ]?innings?|overtime|extra[- ]?time|sudden[- ]?death|golden[- ]?goals?|stun|stuns|stunned|stunning|stunner|shock|shocks|shocked|shocking|crush\w*|outlast\w*|outclass\w*|outplay\w*|overpower\w*|overwhelm\w*|outgun\w*|outmuscl\w*|outduel\w*|outscor\w*|outpoint\w*|outbox\w*|outfight\w*|outfought|prevail\w*|surviv\w*|relegat\w*|overcome|overcomes|overcoming|overcame|dominat\w*|defeat\w*|beat\w*|edge\w*|pip(?:s|ped|ping)?|dispatch\w*|(?:takes?|taking|took) down|sinks?|sank|sunk|(?<!ups and )downs|downed|holds?[- ]?off|held[- ]?off|hold(?:s|ing)?[- ]?on|held[- ]?on|hang(?:s|ing)?[- ]?on|hung[- ]?on|(?:sees?|saw|seen|seeing) out (?:a |an |the )?(?:[\w-]+ )?(?:win|victory|result|points?|lead)|sees?[- ]?off|saw[- ]?off|fends?[- ]?off|fended[- ]?off|rout|routs|routed|top(?:s|ped)|toppl\w*|trounc\w*|demolish\w*|destroy\w*|dismantl\w*|humiliat\w*|embarrass\w*|capitulat\w*|choke\w*|collaps\w*|obliterat\w*|annihilat\w*|decimat\w*|vanquish\w*|pulveri[sz]\w*|thrash\w*|thump\w*|pummel\w*|steamroll\w*|drub\w*|smash\w*|wallop\w*|spank\w*|maul\w*|clobber\w*|shellac\w*|overrun\w*|overran|brush(?:es|ed|ing)?[- ]?aside|(?:runs?|running|ran) (?:riot|rampant)|(?:runs?|running|ran)[- ]?away[- ]?with|(?:runs?|running|ran) rings (?:a)?round|to the sword|(?:tak(?:e|es|ing)|took)[- ]?(?:the[- ]?)?che(?:ck|qu)ered[- ]?flag|cross(?:es|ed|ing)?[- ]?(?:the[- ]?)?(?:finish[- ]?)?line[- ]?first|hammer(?:ed|ing)|batter(?:ed|ing)|cruise(?:s|d)?|canter(?:s|ed|ing)?|pull(?:s|ed|ing)?[- ]?away|(?:makes?|made|making) (?:light|hard) work of|prov(?:e|es|ed|ing) too (?:strong|good|much)|(?:ha(?:ve|s|d)|having) too much (?:class |quality |firepower |pace |power |strength )?for|(?:gets?|getting|got) the better of|(?:gets?|getting|got) the job done|(?:gets?|getting|got) over the line|put(?:s|ting)?[- ]?(?:the |this |that )?(?:game|tie|match|contest|result|series|final|derby|affair)s? to bed|put(?:s|ting)?[- ]?(?:the |this |that |it |a |an )?(?:[\w'’-]+ )?(?:beyond (?:all )?(?:doubt|reach)|out of (?:sight|reach))|(?:grind(?:s|ing)?|ground)[- ]?out (?:a |an |the )?(?:win|victory|result|draw|points?)|ek(?:e|es|ed|ing)[- ]?out (?:a |an |the )?(?:win|victory|result|draw|points?)|(?:eas(?:e|es|ed)|power(?:s|ed)?|breez(?:e|es|ed)|coast(?:s|ed)?|stroll(?:s|ed)?|waltz(?:es|ed)?|roll(?:s|ed)?|blow(?:s|n)?|blew|battl(?:e|es|ed|ing)|grind(?:s|ing)?|ground|get(?:s|ting)?|got)[- ]?past|(?:sneak(?:s|ed)?|snuck|slip(?:s|ped)?|squeez(?:e|es|ed))[- ]?past|squeak(?:s|ed|ing)?[- ]?(?:past|by|through)|scrap(?:e|es|ed|ing)[- ]?(?:past|by|through)|put(?:s|ting)? (?:\d{1,2}|one|two|three|four|five|six|seven|eight|nine|ten) past|triumph\w*|romp\w*|conquer\w*|dethron\w*|upset\w*|upend\w*|spoil(?:s|ed|t|ing)?[- ]?(?:the|their|[\w'’-]+['’]s)[- ]?party|clinch\w*|seals?|sealed|snatch\w*|nick(?:s|ed|ing)?[- ]?(?:it|the (?:win|points?|lead|victory|title|tie)|an? (?:win|winner|point|victory|late (?:winner|goal))|all[- ]?three[- ]?points)|(?:steals?|stealing|stole|stolen)[- ]?(?:it|the (?:win|points?|lead|victory|title|tie)|an? (?:win|winner|point|victory|late (?:winner|goal))|all[- ]?three[- ]?points)|sweep\w*|swept|whitewash\w*|clos(?:e|es|ed|ing)[- ]?out(?: the| a| their| its)? series|oust\w*|eliminat\w*|bow(?:s|ed|ing)?[- ]?out|crash(?:es|ed|ing)?[- ]?out|dump(?:s|ed|ing)?[- ]?out|bundl(?:e|es|ed|ing)?[- ]?out|knock(?:s|ed|ing)[- ]?out|knock out|knock(?:s|ed|ing)? off|sent[- ]?packing|qualif(?:y|ies|ied)|advanc\w*|book(?:s|ed)? (?:(?:their|its|a|his|her) (?:place|spot|berth|ticket|passage)|(?:place|spot|berth|passage))|punch(?:es|ed)? (?:their|its|a|his|her) ticket|reach(?:es|ed|ing)? (?:the )?(?:finals?|semi[- ]?finals?|semis?|quarter[- ]?finals?|quarters?|last[- ]?(?:16|8|4))(?!\s+third)|through to (?:the )?(?:finals?|semi[- ]?finals?|semis?|quarter[- ]?finals?|quarters?|last[- ]?(?:16|8|4))(?!\s+third)|into (?:the )?(?:finals?|semi[- ]?finals?|semis?|quarter[- ]?finals?|quarters?|last[- ]?(?:16|8|4))(?!\s+third)|progress(?:es|ed|ing)? (?:to |into |through to )?(?:the )?(?:finals?|semi[- ]?finals?|semis?|quarter[- ]?finals?|quarters?|last[- ]?(?:16|8|4))(?!\s+third)|crowned (?:world )?champions?|world[- ]?(?:cup[- ]?)?champions?|(?:lift|hoist)(?:s|ed|ing)? (?:the )?(?:world[- ]?cup|trophy)|claim(?:s|ed|ing)? (?:the )?(?:title|crown|trophy|championship|pennant|silverware)|(?:claim|claims|claimed|claiming|take|takes|taking|took|secure|secures|secured|grab|grabs|grabbed|bag|bags|bagged|scoop|scoops|scooped|strike|strikes|struck) (?:the |a |an )?(?:gold|silver|bronze)(?!\s*coast)(?:[- ]?medals?)?(?![-\w])|(?:end|ends|ended|ending|snap|snaps|snapped|snapping|halt|halts|halted|halting|break|breaks|breaking|broke|broken)(?:\s+[\w'’.-]+){0,3}?\s+(?:unbeaten|unbeatable|winless|perfect|flawless)[- ]?(?:run|streak|start|record)|(?:unbeaten|unbeatable|winless|perfect|flawless)[- ]?(?:run|streak|start|record)(?:\s+[\w'’.-]+){0,2}?\s+(?:ends?|ended|ending|over|snapped|halted|broken|done)|leads?|leaders?|winning|winners?|wins|won|win|victory|victories|victorious|(?:comes?|came)[- ]?out on top|(?:ha(?:ve|s|d|ving)|got|get(?:s|ting)?) the last laugh|losing|lose|loses|lost|loss|(?:comes?|came)[- ]?up[- ]?short|(?:falls?|fell)[- ]?short|hat[- ]?tricks?|braces?|(?:triple|double)[- ]?doubles?|no[- ]?hitter|perfect[- ]?games?|empty[- ]?net(?:s|ter|ters)?|shut[- ]?outs?|\d{1,3}[- ]?unanswered|unanswered[- ]?(?:points?|runs?|goals?|scores?|buckets?)|blow[- ]?outs?|shoot[- ]?outs?|straight[- ]?sets?|on penalt(?:ies|y kicks)|goalless|scoreless|blank(?:s|ed|ing)|\d{1,2}[- ]?nil|nil[- ]?(?:\d{1,2}|nil|all)|clean[- ]?sheets?|deadlock\w*|stalemate\w*|salvag\w*|rescu\w*|consolat\w*|(?:claim(?:s|ed|ing)?|tak(?:e|es|ing)|took) the spoils|share(?:s|d)? the spoils|share(?:s|d)? the points|share(?:s|d)? the honou?rs|a point (?:apiece|each)|honou?rs even|held to an? (?:[\w-]+ )?draw|play(?:s|ed|ing)?[- ]?out an? (?:[\w-]+ )?draw|settl(?:e|es|ed|ing)[- ]?(?:it\b|the[- ](?:tie|match|game|contest|series|final|derby|affair)s?)|settl(?:e|es|ed|ing) for (?:a|an|the) (?:draw|point|stalemate)|all[- ]?square|equali[sz]\w*|level(?:l)?ers?|(?:draws?|drew|drawing)[- ]?level|level(?:s|led|ling)?[- ]?(?:it\b|things up|the (?:scores?|tie|match|contest|derby|affair|aggregate))|go[- ]?ahead (?:goal|run|homer|home[- ]?run|score|basket|bucket|touchdown|header|strike)s?|own[- ]?goals?|worldies?|golazos?|wonder[- ]?goals?|screamers?|grand slam|send(?:s|ing)?[- ]?off|sent[- ]?off|sees?[- ]?red|saw[- ]?red|red card|marching[- ]orders|all three points|bowl(?:s|ed|ing)[- ]?out|all[- ]?out for|chas(?:e|es|ed|ing)[- ]?down|chas(?:e|es|ed) \d{2,3}\b|super[- ]?over|defend(?:s|ed|ing)? \d{2,3}\b|\d{2,3}\/(?:10|\d)\b|\d{2,3} for \d\b|five[- ]?for\b|fifer|wicket haul|skittl\w*|TKO|KOs?|KO'd|stops|stopped|def(?=\.)|retain(?:s|ed)|finish(?:es|ed)|starch\w*|submission\w*|submit(?:s|ted|ting)|tap(?:s|ped|ping)?[- ]?out|(?:unanimous|split|majority)[- ]?decision)\b/i;
