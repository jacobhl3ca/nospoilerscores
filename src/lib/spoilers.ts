...
//     waltz(?:es|ed)?)[- ]?past" catches the SAME comfortable-win framing "cruise(?:s|d)?" covers
...
//     Getafe", "Real Madrid coast past Alaves", "United powered past City") — each of which
//     names the side that was beaten comfortably yet slipped past every existing verb and
//     carries no digits for SCORE_RX. It is anchored to the mandatory trailing "past" exactly
//     like the "brush …aside"/"hold …off"/"see …off" idioms below/above, and that anchor is
//     what makes the otherwise-common bare verbs safe: "power"/"ease"/"coast" cannot fire alone,
//     so "power ranking", "star power", "at ease", "West Coast", "coast to coast" and "years
//     past" all pass through untouched. Each verb lists only the forms recap titles use
//     (ease/eases/eased, power/powers/powered, breeze/breezes/breezed, coast/coasts/coasted,
//     stroll/strolls/strolled, waltz/waltzes/waltzed); the rarer -ing forms are left alone like
...
//     the worker's copy.
//     "consolation\w*" catches the consolation-goal reveal soccer/hockey recap titles lean on constantly
...
//     cites "salvage pride with a consolation"). Note the anchor is the full stem "consolation", NOT bare
//     "consol" — this deliberately excludes the comfort VERB console/consoles/consoling/consoled (a gaming
//     "console", "console a teammate") AND "consolidate\w*"/"consolidation" (both diverge from "consolation"
//     after "consol"), so the only strings that match are consolation/consolations/consolatory, each of which
