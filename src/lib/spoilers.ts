...
//     "s?" the singular "world champion" (a boxing/individual world title is itself a result reveal). Its one
//     benign collision — a spoiler-free "who will be world champions?" final preview — errs to the same
//     over-hide-is-safe side as "crowned champions?" above (a masked title costs a tap to reveal; a leaked
//     one breaks the whole promise), and is especially worth catching as the World Cup reaches its final.
//     Byte-identical to the worker's copy.
