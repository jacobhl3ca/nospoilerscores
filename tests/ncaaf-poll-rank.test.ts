...
test("the college chip is not hidden on finished cards", () => {
  ...
  const branch = card.slice(card.indexOf('} else if (game.sport === "ncaaf" || game.sport === "ncaah" || game.sport === "ncaawh" || game.sport === "ncaavb" || game.sport === "ncaawsoc" || game.sport === "ncaamsoc" || game.sport === "ncaabase" || game.sport === "ncaasoft") {'), card.indexOf("} else if (team.rank != null"));
