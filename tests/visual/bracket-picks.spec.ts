...
// MLB column → the "Playoff bracket" button → the picture's own cover → Picks.
//
...
async function openPicks(page: Page, revealed = false) {
  ...
  await page.getByRole("button", { name: /Playoff (picture|bracket)/ }).first().click({ timeout: 20_000 });
